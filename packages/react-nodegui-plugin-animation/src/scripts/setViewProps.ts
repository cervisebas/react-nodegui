...
import { WidgetAttributesMap } from '../interfaces/WidgetAttributesMap';
import { WindowFlagsMap } from '../interfaces/WindowFlagsMap';
...
export function setViewProps<Signals extends object>(
  ...
  newProps: ViewBaseProps<Signals>,
  oldProps: ViewBaseProps<Signals>,
) {
  const setter: ViewBaseProps<Signals> = {
    ...
    set on(listenerMap: Partial<WidgetEventListeners | Signals>) {
      const listenerMapLatest = Object.assign({}, listenerMap) as Record<
        string,
        never
      >;
      ...
      Object.entries(oldListenerMap).forEach(([eventType, oldEvtListener]) => {
        ...
        if (oldEvtListener !== newEvtListener) {
          widget.removeEventListener(eventType as never, oldEvtListener);
        } else {
